...
def audio_folder_to_pyarrow(
  ...
  # read audio
  samples, expected_sr = soundfile.read(audio_files[0])
  ...
  def do_batch(batch: Iterable[Path]):
    ...
    for file_path in batch:
      x, sr = soundfile.read(str(file_path))
      assert sr == expected_sr, f"wrong sampling rate detected at {file_path}"
      assert len(x) == len(samples), f"wrong clip length detected at {file_path}"
      audio_data.append(x)
