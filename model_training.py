...
def get_confusion_matrix(y_true, y_pred, labels: list[str]):
    ...
    confusion matrix
    """
    return ConfusionMatrixDisplay.from_predictions(np.argmax(y_true, axis=1), np.argmax(y_pred, axis=1), display_labels=labels).figure_


def display_confusion_matrix(y_target, y_predicted, labels=None, plot_path=None, is_show_plot=False):
    """
...
def run_model_training(config: Config):
    ...
    display_confusion_matrix(y_true, y_pred, labels=list(class_dict.keys()), plot_path=CM_FIG_PATH, is_show_plot=True)

    # old cm
    #cm_fig = get_confusion_matrix(y_true, y_pred, labels=list(class_dict.keys()))
